...
We get cell expression signature matrices
```{r}
...
ggimmune = ggplot(immune_scores.long %>% filter(!group%in%c("LCNEC","SCLC"),!is.na(group)), aes(x=Method,y=Score,fill=group) ) + 
  ...
  theme_classic() + scale_fill_manual(values=c(arc4,type5)) +ylab("Immune score")+xlab("") + guides(fill="none") + facet_wrap(.~group) + 
  theme(axis.text.x = element_text(angle=90,hjust = 1,vjust=0.5))

ggimmune

ggsave(filename="/data/lungNENomics/work/alcalan/Figures/ExtendedDataFig_1B_Immune.svg",ggimmune,height =2.5*2,width=3*2)
```
...
fibro.tib$group = factor(fibro.tib$group,levels=c("LCNEC","SCLC","sc-enriched","Ca A2","Ca A1","Ca B"))

ggfibro = ggplot(fibro.tib %>% filter(!group%in%c("LCNEC","SCLC"),!is.na(group)), aes(x=group,y=Score,fill=group) ) + geom_violin(col=NA) + #geom_boxplot(width=0.1,fill="white") +
  geom_beeswarm(col="white",shape=21,size=1)+
  ggpubr::stat_compare_means(comparisons = list(c("sc-enriched","Ca A2"),c("Ca A2","Ca A1"),c("Ca A1","Ca B"),
                                        c("sc-enriched","Ca A1"),c("sc-enriched","Ca B"),
                                        c("Ca A2","Ca B")),
                     method = "wilcox.test",
                     #label.y = c(0.16,0.15,0.17,0.18)*100,
                     label = "p.signif",hide.ns = T , bracket.size = 0.6,tip.length = 0,vjust = 0.6) + 
  #ylim(c(0,20))+
  theme_classic() + scale_fill_manual(values=c(arc4,type5)) +ylab("Fibroblast score")+xlab("") + guides(fill="none") + facet_wrap(.~method,scales = "free_y") + 
  theme(axis.text.x = element_text(angle=90,hjust = 1,vjust=0.5))

ggfibro

ggsave(filename="/data/lungNENomics/work/alcalan/Figures/ExtendedDataFig_1C_Fibro.svg",ggfibro,height =2.5*2,width=3*2)
```