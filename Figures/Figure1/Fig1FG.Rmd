...
arc.features = tibble(sample = colnames(gene_TPM_matrix.LN_TCGA)[-(1:2)],
                      Tcell.sig.TPM   = colMeans( log2((gene_TPM_matrix.LN_TCGA %>% filter(gene_name%in% immune_arc_genes$Tcell ) )[,-c(1:2)]+1) ),
                      Myeloid.sig.TPM = colMeans( log2((gene_TPM_matrix.LN_TCGA %>% filter(gene_name%in% immune_arc_genes$Myeloid ) )[,-c(1:2)]+1 )),
                      Stroma.sig.TPM  = colMeans( log2((gene_TPM_matrix.LN_TCGA %>% filter(gene_name%in% immune_arc_genes$Stroma ) )[,-c(1:2)]+1 ))
                      )
...
# add LUAD and LUSC. 497 samples, 534 LUAD, 502 LUSC => total 1533
arc.features$group[497+1:534] = "LUAD"
arc.features$group[497+534+1:502] = "LUSC"
...
arc.features.df.long = arc.features.df %>% filter(!is.na(group)) %>% pivot_longer( cols=c(Tcell:Stroma),names_to = "Feature",values_to = "Score" ) 
arc.features.df.long$Feature = factor(arc.features.df.long$Feature,levels=c("Tcell","Myeloid","Stroma") )
```


#### Clustering
We perform UMAP following Combes et al. for vizualisation, and leiden clustering to group samples

```{r}
## 3-feature scores
library(umap)
umap3f = umap(arc.features.df[,2:4])

leiden_clustering = function( latent_dat,knearest=100, Resolution=0.1){
  set.seed(1234)
  PCvalues = latent_dat
  dim(PCvalues)
...
leiden_n100_r0.1 = leiden_clustering(arc.features.df[,2:4])
```

#### Labelling of clusters
We first plot the clusters and their marker values
```{r}
ggplot(bind_cols(as_tibble(umap3f$layout),arc=arc.features.df$group,cluster=factor(leiden_n100_r0.1) ),
       aes(x=V1,y=V2,fill=arc,fill=cluster) ) + 
  geom_point(shape=21) + theme_classic() # 6 communities as in original paper

### add clustering info 
arc.features.df.long = bind_cols(arc.features.df,cluster=paste0("C",leiden_n100_r0.1)) %>% 
  filter(!is.na(group)) %>% pivot_longer( cols=c(Tcell:Stroma),names_to = "Feature",values_to = "Score" ) 
arc.features.df.long$Feature = factor(arc.features.df.long$Feature,levels=c("Tcell","Myeloid","Stroma") )

ggplot( arc.features.df.long , aes(x=cluster,y=Score,fill=cluster)) + 
  geom_violin(scale = "width") +geom_boxplot(width=0.1,fill="white") + facet_wrap(.~Feature)+ theme_classic()
```
Based on the plots, we identify the following correspondance:
      T M S
 cl5: + + + => 1. Immune stromal (90 90 80)
 cl2: +(75) +(75) ~(50) => 2. Immune rich (80 80 35)
 cl6: ~(50) ~(60) +(80) => 1. 4. Myeloid centric (50 60 80)
 cl4: - - + => 5. Immune desert  (20 20 70)
 cl3: -(30) -(25) -(30) => 6. T cell centric (45 30 40)
 cl1: - - - => 6. Immune stroma desert (15 10 20)
...
arc.features.df.long$cluster.label[arc.features.df.long$cluster=="C5"] = "Immune_stromal"
arc.features.df.long$cluster.label[arc.features.df.long$cluster=="C2"] = "Immune_rich"
arc.features.df.long$cluster.label[arc.features.df.long$cluster=="C6"] = "Myeloid_centric"
arc.features.df.long$cluster.label[arc.features.df.long$cluster=="C4"] = "Immune_desert"
arc.features.df.long$cluster.label[arc.features.df.long$cluster=="C3"] = "T_cell_centric"
...
FigS61 = ggplot( arc.features.df.long %>% filter(Feature %in% c("Tcell","Myeloid","Stroma")) , 
        aes(x=cluster.label,y=Score,fill=cluster.label)) + geom_violin(scale = "width") +  geom_boxplot(width=0.2,fill="white")+
  scale_fill_manual(values=immarc6) + facet_grid(.~Feature) +
  theme_classic() + theme(axis.text.x=element_blank(),axis.ticks.x = element_blank(),axis.line.x=element_blank()) +xlab("")

ggsave(filename="../../../Figures/Supplementary_Figure_S61_immune.svg",FigS61,height=2.5,width=3*2)
ggsave(filename="../../../Figures/Supplementary_Figure_S61_immune.pdf",FigS61,height=2.5,width=3*2)
```
...
write_csv(arc.features.df.wide,file = "/data/lungNENomics/work/alcalan/tomoveto_Descriptive_manuscript_data/RNAseq/TableS44_immune_archetypes.csv")
```
...
fisher.test(Immarc6.tib2[1:3,],simulate.p.value = T)# significant differences between A1 A2 and B
fisher.test(Immarc6.tib2[1:2,],simulate.p.value = T)# no diff between A1 and A2
fisher.test(Immarc6.tib2[c(1,3),],simulate.p.value = T)# significant differences between A1 and B
fisher.test(Immarc6.tib2[c(1,4),],simulate.p.value = T)# significant differences between A1 and SC
fisher.test(Immarc6.tib2[c(2,3),],simulate.p.value = T)# significant differences between A2 and B
fisher.test(Immarc6.tib2[c(2,4),],simulate.p.value = T)# significant differences between A2 and SC
fisher.test(Immarc6.tib2[c(3,4),],simulate.p.value = T)# significant differences between A2 and SC
```
...
ggsave(ggimmunearc,filename = "Fig1F_immunearcs.svg",height=3,width=5)
...
ggsave(filename="Fig1G_raw.svg",Fig1G_A1 + Fig1G_A2 + Fig1G_B + Fig1G_sce,width=2.5*2,height=2.5*2)
