...
# Figure 2 panel B: Driver mutations associated with archetypes or types - oncoplot
...
Plot Fig S11 and write results presented in Table S17
```{r}
shatt.tot = shatt
...
sample_order <- left_join(oncoplot,var_cli[,c("sample_id","group","type")]) %>% arrange(group,MEN1,ARID1A,EIF1AX,ATM,`HLA-C`,`HLA-A`,ASXL1,BRAF,CLIP1,SGK1,
                                                                                        `-1p13.3`,  `-3p`, `-3q`,  `+5q`,  `+5p`, `+7p` , `-11q13.1`, `+14p` ,WGD,`Shattered regions`,Epigenetic.regulatory.genes,type) %>% pull(sample_id)#oncoplot$sample_id # Set sample order
#sample_order <- oncoplot$sample_id # Set sample order
...
patchwork
ggsave(patchwork,file="EDFig2c_Oncoplot_drivers_extended_version_row_by_arch.pdf", h=9,w=8)
ggsave(patchwork,file="EDFig2c_Oncoplot_drivers_extended_version_row_by_arch.svg", h=9,w=8)
```
...
```{r}
oncoplot_temp$SNV_event_driver_gene <- ifelse(apply(oncoplot_temp[, 2:11], 1, function(x) any(x != "WT")), 1, 0 ) # OK
oncoplot_temp$CNV_event <- ifelse(apply(oncoplot_temp[, c(12:19)], 1, function(x) any(x != "WT")), 1, 0 )
oncoplot_temp$WGD_event <- ifelse(oncoplot_temp$WGD != "WT", 1, 0)
oncoplot_temp$shattered_event <- ifelse(oncoplot_temp$`Shattered regions` != "WT", 1, 0)
oncoplot_temp$Epigenetic_regulatory_gene_event <- ifelse(oncoplot_temp$Epigenetic.regulatory.genes != "WT", 1, 0)

summary_event <- oncoplot_temp[,c(1,23:28)]
summary_event$several_events <- ifelse(apply(summary_event[, 3:7], 1, function(x) sum(x) > 1), 1, 0 )
summary_event$any_events <- ifelse(apply(summary_event[, 3:7], 1, function(x) sum(x) == 0), 1, 0 )

summary_event[summary_event$several_events == 1, 3:7] = 0
...
palette_driver_event <- c("any_events" = "#7d7d7d", "SNV_event_driver_gene" = "#4EB265",   "CNV_event"= "#FF9F00",  
                          "WGD_event" = "#C4DE54", "shattered_event" = "#81031D",  "Epigenetic_regulatory_gene_event" = "#4EC6D0",   "several_events" = "#DC050C" )
...
ggsave(plot_e,filename="EDFig2e_Barplot_percentages_of_sampels_with_imp_genomic_event.pdf", h=3,w=5)
ggsave(plot_e,filename="EDFig2e_Barplot_percentages_of_sampels_with_imp_genomic_event.svg", h=3,w=5)
